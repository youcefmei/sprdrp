...
public class StateDAO implements IDAOObservable<State> {
    ...
    public State findById(int id) {
        ...
        try {
            PreparedStatement preparedStatement = conn.prepareStatement(
                    "SELECT * FROM State WHERE id_state = ?", PreparedStatement.RETURN_GENERATED_KEYS);
    ...
    public State create(State state ) {
        ...
        try {
            PreparedStatement pStatement = conn.prepareStatement(
                    "INSERT INTO State(`name`,`code`) VALUES (?,?) ",Statement.RETURN_GENERATED_KEYS);
    ...
    public boolean update(State state) {
        try {
            PreparedStatement preparedStatement = conn.prepareStatement(
                    "UPDATE State SET name = ? , code = ? WHERE id_state = ?",
    ...
    public boolean delete(int id) {
        try {
            PreparedStatement pStatement = conn.prepareStatement("DELETE FROM State WHERE id_state = ? " ,
                    PreparedStatement.RETURN_GENERATED_KEYS);
